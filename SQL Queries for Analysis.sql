...
--Calculate the Average Discount Given per Category:
SELECT category, AVG(discount_percent) AS avg_discount
FROM df_orders
GROUP BY category
ORDER BY avg_discount DESC;

--Identify the Top 5 Customers with the Highest Total Purchases:
SELECT customer_id, SUM(sale_price) AS total_purchases
FROM df_orders
...
LIMIT 1;
 ...
 --Calculate the Total Number of Orders Each Month:
SELECT EXTRACT(YEAR FROM order_date) AS order_year,
       EXTRACT(MONTH FROM order_date) AS order_month,
